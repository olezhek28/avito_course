...
func TestImplementation_GetEventListForDay(t *testing.T) {
	...
	var (
		...
		req = &desc.GetEventListForDayRequest{
			Date: timestamppb.New(startDate),
		}
	...
	tests := []struct {
	...
	}{
		{
			...
			eventRepositoryMock: func(mc *minimock.Controller) repository.EventRepository {
				...
				mock.GetEventListForDayMock.Expect(ctx, startDate).Return(eventsRepoRes, nil)
				return mock
			},
		},
		{
			...
			eventRepositoryMock: func(mc *minimock.Controller) repository.EventRepository {
				mock := repoMocks.NewEventRepositoryMock(mc)
				mock.GetEventListForDayMock.Expect(ctx, startDate).Return(nil, repoErr)
				return mock
			},
		},
	}
