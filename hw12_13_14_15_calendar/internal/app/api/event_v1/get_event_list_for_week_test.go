...
func TestImplementation_GetEventListForWeek(t *testing.T) {
	...
	var (
		...
		req = &desc.GetEventListForWeekRequest{
			WeekStart: timestamppb.New(startDate),
		}
	...
	tests := []struct {
	...
	}{
		{
			...
			eventRepositoryMock: func(mc *minimock.Controller) repository.EventRepository {
				...
				mock.GetEventListForWeekMock.Expect(ctx, startDate).Return(eventsRepoRes, nil)
				return mock
			},
		},
		{
			...
			eventRepositoryMock: func(mc *minimock.Controller) repository.EventRepository {
				mock := repoMocks.NewEventRepositoryMock(mc)
				mock.GetEventListForWeekMock.Expect(ctx, startDate).Return(nil, repoErr)
				return mock
			},
		},
	}
